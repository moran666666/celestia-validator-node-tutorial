...
```shell
cd ~                                                           # 回到家目录
wget "https://golang.org/dl/go1.19.1.linux-amd64.tar.gz"       # 下载go1.19.1压缩包
sudo rm -rf /usr/local/go                                      # 删除原有的go
sudo tar -C /usr/local -zxvf "go1.19.1.linux-amd64.tar.gz"     # 解压新的go
rm -rf go1.19.1.linux-amd64.tar.gz                             # 解压完成后可删除下载的go1.19.1
...
# 检查golang版本, 有1.19.1版本输出表示环境设置成功
go version
```
...
git checkout tags/v0.11.0 -b v0.11.0                          # 切换到官方要求的项目仓库分支
...
celestia-appd init $MONIKER --chain-id mocha                        # 会生成~/.celestia-app/配置目录
cp ~/networks/mocha/genesis.json ~/.celestia-app/config             # 拷贝genesis.json创世文件到~/.celestia-app/配置目录中
...
sed -i -e "s|^persistent_peers *=.*|persistent_peers = \"$PEERS\"|g" $HOME/.celestia-app/config/config.toml  # 修改配置文件, 将官方的启动节点添加到里面
sed -i -e "s|^seed_mode *=.*|seed_mode = true|" $HOME/.celestia-app/config/config.toml  
```
...
SNAP_NAME=$(curl -s https://snaps.qubelabs.io/celestia/ | egrep -o ">mocha.*tar" | tr -d ">")   # 获取将要下载的区块数据文件名称
wget https://snaps.qubelabs.io/celestia/${SNAP_NAME}                                            # 下载快照文件, 下载视网络速率情况，可能需要很长时间
...
celestia-appd tx staking delegate $OP_WALLET $AMOUNT --from=$VALIDATOR_WALLET --chain-id=mocha     # 质押多少币到操作员地址的验证节点上(无法上链, 可加大gas、fees)
```
...
git checkout tags/v0.6.0                                      # 切换到官方要求的项目仓库分支
make install                                                  # 编译，并自动安装到bin目录下
...
```shell
celestia bridge init --core.ip 127.0.0.1 --core.grpc.port 9090                          # 会生成~/.celestia-bridge/配置目录
...
EVM_ADDRESS="0xf2Bb1xxxxxxxxxxxxxxxxxxxxxxx"                # 任意一个eth钱包地址
ORCHESTRATOR_ADDRESS="celestia1dxxxxxxxxxxxxxxxxxxxxxx"     # 新创建的钱包地址
celestia-appd tx staking create-validator \                 # 无法上链, 可加大gas、fees
    --amount=$AMOUNT \
